module.exports = function(dispatchNotification,
  ...
  return async function (event, handlerName, hnadlerFunction) {
    ...
    try {
      ...
      const isIdempotent = await eventHelperRepository.checkIdempotency(fh.getSafeValue('originalEventNumber', event), handlerName);
      ...
      await eventHelperRepository.recordEventProcessed(fh.getSafeValue('originalEventNumber', event), handlerName);
